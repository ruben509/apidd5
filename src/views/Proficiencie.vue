<template>
  <div class="espaço5">
     <div class="tamanho">References
        <div class="cor2" v-for="(item,index) in this.changes.references" :key="index">
            <h3>{{item.name}}</h3>
            <button @click="anotar(item.name)">+</button>
                <div class="text"> <h3>{{item.desc}}</h3>
                </div>
    ...
    <div class="tamanho">Type
        <div class="cor2" v-for="(item,index) in this.changes.references" :key="index">
            <h3>{{item.type}}</h3>
            <button @click="anotar(item.type)">+</button>
        </div> 
    </div>
     <div class="tamanho">Type2
        <div class="cor2">
...
export default {
  ...
  carregaclasses(ur){
        axios.get('https://www.dnd5eapi.co/api/proficiencies/'+ur)
            .then(res=>{
